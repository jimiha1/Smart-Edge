...
4. **前台追踪改为 active 窗口权威判定（第二次实机复现后）**：实机日志（2026-09-05 19:47:26-29，中信银行 app 启动）证实「最后一条窗口事件说了算」的前台追踪被系统杂音污染——应用启动动画期间，桌面/负一屏（hiboard）再次自我宣告，把已切到应用的前台抢回「桌面」，把手随即显示在应用上（全程门控判定本身无误，是输入数据错了；日志中 hiboard↔launcher 来回倒手 13 次）。终稿改为：窗口事件仅作触发，前台真值取无障碍窗口列表中 `isActive` 窗口的包名——`AccessibilityWindowInfo` 本身不暴露包名，需经 `getRoot().packageName` 读取，因此服务配置另需 `canRetrieveWindowContent="true"`（该权限同时激活了原本因配置缺失而失效的沉浸模式检测）；列表不可用时回退事件包名；埋点 #1 相应增加 `active=` 字段记录权威值与事件值的分歧。与第 3 条共用 `flagRetrieveInteractiveWindows` 依赖。
