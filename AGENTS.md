...
- **Overlay/runtime**: `FloatingPanelService` (panel lifecycle + handle), `SidePanelView`, `EdgeHandleView`/`NotchHandleView`, `ActionDispatcher` (all handle/gesture actions route through this).
...
- No DI framework, no Coroutines/Flow libraries declared at app level, no Compose — match the existing plain-Kotlin + ViewBinding + Handler/coroutine style.
